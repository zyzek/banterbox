...
def get_rooms(request):
    ...
    rooms = []
    for user_enrolement in UserUnitEnrolment.objects.filter(user_id = user.id):

        unit = Unit.objects.get(id=user_enrolement.unit_id)
...
def comment_post(request, room):
    ...
    rooms = []
    for user_enrolement in UserUnitEnrolment.objects.filter(user_id = user.id):
        if room == Room.objects.get(unit_id=user_enrolement.unit_id):
